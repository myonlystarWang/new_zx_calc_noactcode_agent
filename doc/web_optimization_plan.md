...
| Step 12 | A/B 方案并排对比 + Header IA 对齐 | 🟡 P2 | 3-4 h | Step 9（对比依赖多套方案） | ✅ 已完成 |
| 专项优化 | 全站 CPU 负载与 GPU 渲染性能深度治理 | 🔴 P0 | 1 h | 无 | ✅ 已完成 |
...
- 经深度评估与实测：全景资料库顶部已具备清晰的一级主分类 Tab（极致属性 / 职业技能 / 职业状态 / 副本 BOSS）及二级难度与分类胶囊筛选，且 URL 已完全支持 Hash 路由深度定位与浏览器前进后退。页面层级扁平高效，收尾阶段保持界面整洁，免于堆砌冗余面包屑。

#### 12.3 Header 导航 vs 首页卡片 IA 对齐

- Header 的「全景战斗资料库」按钮增加了下拉菜单，主体按钮直达资料库，尾部 Caret 按钮点击/Hover 触发下拉 Portal 菜单，列出 4 个子入口（极致属性攻略、职业技能速查、职业状态一览、副本 BOSS 速查），与首页 5 张功能卡片入口完全对齐。

### 验收目标

- [x] 选中两套方案，并排对比页面正确展示差值（`CompareDialog.tsx` + `presetCompare.ts`）
- [x] 差值用颜色+百分比标注（较高一侧标绿并显示差值与增幅，相等不标）
- [x] Header 资料库 Tab 展开可看到 4 个子入口并实现任意页面秒级跳转
- [x] 资料库深层页面通过主分类 Tab + 胶囊筛选 + Hash 路由闭环支持快速逐层切换与直达
- [x] 依赖 Step 9 多方案数据，弹窗纯只读不污染当前计算器状态，支持 A/B 交换与一键 Esc 关闭
- [x] 以上功能不影响现有功能，全量构建 0 错误

---

## 专项优化：全站 CPU 负载与 GPU 渲染性能深度治理

> **目标**：彻底解决全站（包含首页、计算器、BOSS速查）打开时 Chrome 进程 CPU 占用高的问题，实现前后台极低空载功耗

### 治理点与实施内容

1. **首页 Canvas 彗星粒子动画重构**：
   - 移除 30-step 尾迹循环中的 `ctx.shadowBlur = 12` 软件高斯模糊栅格化，改用 GPU 硬件抗锯齿双通道笔触（外晕+内芯），消除每秒数千次高斯模糊计算；
   - 尾迹步数从 30 步压缩至 10 步，帧率节流至约 40fps，每秒发往 GPU 的绘制指令减少 80% 以上；
   - **8 秒空闲休眠（Idle Sleep）**：用户停留在首页超过 8 秒无键鼠/触屏操作，Canvas 动画自动静默暂停；一旦检测到用户活动（mousemove/scroll/keydown），即时无缝唤醒；
   - **Page Visibility API**：切到其他标签页或浏览器最小化时彻底终止 `requestAnimationFrame`，后台 CPU 保证为 0%。

2. **首页打字机提示词纯 DOM 化**：
   - 移除 `placeholder` React State，改为直接赋值 `inputRef.current.placeholder = ...`；
   - 彻底消除了每 40ms 触发一次、整页 900 行组件的全量 React VDOM Diffing，React 组件空载重渲染从 15~20 次/秒归零。

3. **计算器页大面积高斯模糊动画消除**：
   - 移除 `ResultsSection.tsx` 中 `blur-[40px] animate-glow` 与 `animate-pulse-slow` 持续 CSS 动画；
   - 将大面积 40px 高斯模糊转为 GPU 静态纹理缓存环境光，彻底消除 GPU 进程对大面积模糊层每秒 60 次的持续重栅格化。

4. **全站 Header NEW 角标无限脉冲动画消除**：
   - 移除 `Header.tsx` 中版本提示 `animate-pulse`，杜绝全局常驻组件无休止唤醒 Chromium Compositor 合成线程。
...
| P3-3 | **彗星 Canvas rAF 优化**（visibilitychange 暂停循环） | 优化方案 §四 🟡 / 审计报告修正降为 🟢 P3-23 | 已在「全站 CPU 负载与 GPU 渲染性能深度治理」中超额完成（含 Page Visibility 休眠、8秒空闲暂停、双通道笔触优化） | ✅ 已完成 |
