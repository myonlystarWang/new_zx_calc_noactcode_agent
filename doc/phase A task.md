...
## [/] 阶段B：数据 Schema 与 Profile 覆盖 (进行中)
...
- `[ ]` **B-2：定义玩家 Profile 的 `skillOverrides` 运行时融合机制**
  - [ ] 在共享包中实现 `PlayerSkillOverride` 接口与 deep-merge 覆盖助手函数
  - [ ] 在 Actor 的构造器中实现玩家个性化 CD/施法时间/Buff 覆盖混入
- `[ ]` **B-3：扩充 Boss 数据与 HP 配置机制**
  - [ ] 为 `dungeons_monsters.json` 架构中的各 Boss 补充 `MonsterHealth` 字段，填入 T19-T21 和天帝宝库的主流 Boss 血量
  - [ ] 支持模拟器场景 input 中允许临时覆盖 Boss 血量
- `[ ]` **B-4：开发自动化数据校验工具（Validator Script）**
  - [ ] 编写校验脚本，检查技能、Buff/Debuff/功能技能、互斥组 `ExclusiveGroup`、优先级 `Priority`、覆盖策略 `ExclusivePolicy` 等配置完整性
- `[ ]` **B-5：阶段B 回归与可用性校验**
  - [ ] 编写测试脚本校验 Profile 覆盖以及数据校验工具功能正常，确保无编译和逻辑错误
